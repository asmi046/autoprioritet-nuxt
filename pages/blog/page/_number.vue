  ...
  <section class="blog_section">
    <div class="container category_container">
      ...
      <div class="main-blog__wrapper page-blog__wrapper">
        ...
        >
          <blog-element :img="item.featured_image_url" :title="item.title.rendered" />
        </nuxt-link>
      </div>
      <pagination :total-count="totalCount" :inpage="inPage" :current-page="currentPage" />
    </div>
  </section>
</template>
...
export default {
  async asyncData ({ params, $axios, route }) {
    ...
    const inPage = 9
    const postContent = await $axios.get('http://head.xn--80aejla8abgjcqhb.xn--p1ai/wp-json/wp/v2/posts/?categories=16&per_page=' + inPage + '&page=' + params.number)
